...
event_response_t svc_callback(vmi_instance_t vmi, vmi_event_t *event)
{
    (void)vmi;
    printf("SVC event\n");
...
int main (int argc, char **argv)
{
    ...
    vmi_event_t svc_event = {0};
    svc_event.version = VMI_EVENTS_VERSION;
    svc_event.type = VMI_EVENT_PRIVILEGED_CALL;
    svc_event.callback = svc_callback;

    // register event
    if (vmi_register_event(vmi, &svc_event) == VMI_FAILURE)
        goto error_exit;
    ...
    retcode = 0;
error_exit:
    vmi_clear_event(vmi, &svc_event, NULL);
